...
def send_udp_response(pl, probe):
    ...
    send(IP(dst=inet_ntoa(pkt.src), src=inet_ntoa(pkt.dst), ttl=TG, flags=frag_bit) / ICMP(code=3, type=3) /
         IP(dst=inet_ntoa(pkt.dst), src=inet_ntoa(pkt.src), id=pkt.id, ttl=TG - 1) / UDP(dport=pkt.udp.dport,
                                                                                         sport=pkt.udp.sport),
         verbose=0)
...
def send_probe_response(pl, probe):
    ...
    send(IP(id=IPID, dst=inet_ntoa(pkt.src), src=inet_ntoa(pkt.dst), flags=frag_bit, ttl=TG) /
         TCP(sport=pkt.tcp.dport, dport=pkt.tcp.sport, seq=SEQ, ack=ACK, window=W, options=opts, flags=FLAGS),
         verbose=0)
...
def send_probe_response_T1(pl, probe, packet):
    ...
    send(IP(dst=inet_ntoa(pkt.src), src=inet_ntoa(pkt.dst), flags=frag_bit, ttl=TG) /
         TCP(sport=pkt.tcp.dport, dport=pkt.tcp.sport, seq=SEQ, ack=ACK, flags=FLAGS, window=W, options=opts, ),
         verbose=0)
    ...
    for indice_opt in range(0, len(x)):
        if x[indice_opt] == "W":
            ...
            for index in range(indice_opt + 1, len(x)):
                if ((x[index] != "N") and (x[index] != "W") and (x[index] != "M") and (x[index] != "S") and (
                        x[index] != "T") and (x[index] != "L")):
                    w_opt += x[index]
                else:
                    break
...
def print_tcp_packet(pl, destination):
    ...
    if opts.verbose:
        ...
        print("      [+] IP:  source %s destination %s tos %s id %s" % (
            inet_ntoa(pkt.src), inet_ntoa(pkt.dst), pkt.tos, pkt.id))
        print("      [+] TCP: sport %s dport %s flags S seq %s ack %s win %s" % (
            pkt.tcp.sport, pkt.tcp.dport, pkt.tcp.seq, pkt.tcp.ack, pkt.tcp.win))
...
def print_icmp_packet(pl):
    ...
    if opts.verbose:
        ...
        print("      [+] IP:   source %s destination %s tos %s id %s" % (
            inet_ntoa(pkt.src), inet_ntoa(pkt.dst), pkt.tos, pkt.id))
        print("      [+] ICMP: code %s type %s len %s id %s seq %s" % (
            pkt.icmp.code, pkt.icmp.type, len(pkt.icmp.data.data), pkt.icmp.data.id, pkt.icmp.data.seq))
...
def cb_p0f(pl):
    ...
    if (inet_ntoa(pkt.src) == home_ip) and (pkt.p == ip.IP_PROTO_TCP) and (tcp_flags(pkt.tcp.flags) == "S"):
        ...
        if opts.osgenre and opts.details_p0f:
            try:
                pkt_send = module_p0f.p0f_impersonate(
                    IP(dst=inet_ntoa(pkt.dst), src=inet_ntoa(pkt.src), id=pkt.id, tos=pkt.tos) / TCP(
                        sport=pkt.tcp.sport, dport=pkt.tcp.dport, flags='S', seq=pkt.tcp.seq, ack=0), i,
                    osgenre=opts.osgenre,
                    osdetails=opts.details_p0f)
                if opts.verbose:
...
def main():
        ...
        if (not opts.details_p0f):
            for i in range(0, db_size):
                if (db[i][6] == opts.osgenre):
                    print("      WWW:%s|TTL:%s|D:%s|SS:%s|OOO:%s|QQ:%s|OS:%s|DETAILS:%s" % (
                        db[i][0], db[i][1], db[i][2], db[i][3], db[i][4], db[i][5], db[i][6], db[i][7]))
                    exists = 1
        if (opts.details_p0f):
            for i in range(0, db_size):
    ...
    if (not opts.details_p0f and opts.osgenre):
        print(
            " [i] You've only selected p0f OS genre. Details will be chosen randomly every packet from the list bellow")
